...
// The marker for the start of the summary section
...
// Find the end of the col-span-12 div. It ends before success modal or resets.
// A safe way is to find the SUCCESS MODAL start
...
const before = content.substring(0, sectionStart + startMarker.length);
               ...
               <div className="relative z-10 flex flex-col h-full">
                  ...
                  <div className="grid grid-cols-3 gap-4 mb-6">
                    <div className="flex flex-col bg-slate-50/50 p-3 rounded-[var(--radius-md)] border border-slate-100">
                       ...
                       <span className="text-[0.8rem] font-black text-slate-950 tracking-tighter">{formatCurrency(form.subtotal)}</span>
                    </div>
                    <div className="flex flex-col bg-slate-50/50 p-3 rounded-[var(--radius-md)] border border-slate-100">
                       <span className="text-[0.45rem] font-black text-slate-400 uppercase tracking-widest mb-1.5 ">Service Levy (10%)</span>
                       <span className="text-[0.8rem] font-black text-slate-950 tracking-tighter">{formatCurrency(form.sc)}</span>
                    </div>
                    <div className="flex flex-col bg-slate-50/50 p-3 rounded-[var(--radius-md)] border border-slate-100">
                       <span className="text-[0.45rem] font-black text-slate-400 uppercase tracking-widest mb-1.5 ">Fed. VAT (13%)</span>
                       <span className="text-[0.8rem] font-black text-slate-950 tracking-tighter">{formatCurrency(form.tax)}</span>
                    </div>
                  </div>
                            ...
                            <div key={idx} className="bg-slate-50/50 border border-slate-100 p-3 rounded-[var(--radius-md)] space-y-2 relative group">
                              ...
                              <div className="grid grid-cols-5 gap-3">
                                 <div className="col-span-3 relative">
                                    ...
                                    >
                                       {config?.paymentModes?.map((mode: any) => {
                      ...
                      <div className="p-6 bg-indigo-900 border border-indigo-800 rounded-[var(--radius-lg)] flex items-center justify-between shadow-2xl group/parity relative overflow-hidden">
                          ...
                          <div className="flex flex-col relative z-10">
                             ...
                             <span className="text-[0.65rem] font-black text-white uppercase tracking-widest flex items-center gap-2">
                                <ShieldCheck className="w-3.5 h-3.5" /> Variance Discrepancy: <span className={Math.abs(form.amount - form.settlements.reduce((sum: number, s: any) => sum + (s.amount || 0), 0)) > 0.01 ? 'text-rose-400 underline decoration-rose-300' : 'text-white'}>{formatCurrency(form.amount - form.settlements.reduce((sum: number, s: any) => sum + (s.amount || 0), 0))}</span>
                             </span>
                          </div>
                          <div className="text-right relative z-10">
                             <span className="text-[1.1rem] font-black text-white tracking-tighter italic">Total. {form.settlements.reduce((sum: number, s: any) => sum + (s.amount || 0), 0).toLocaleString()}</span>
                          </div>
                      </div>

                      <button 
                         type="submit" 
                         disabled={loading || form.amount <= 0.01 || Math.abs(form.amount - form.settlements.reduce((sum: number, s: any) => sum + (s.amount || 0), 0)) > 0.01}
...
fs.writeFileSync(filePath, before + newSectionContent + after);
console.log("Success: Sales page updated.");
